...
import { Search, ShoppingBag, Heart, User, Menu, X, Package, Globe, Coins } from 'lucide-react';
...
export default function Navbar() {
  ...
  const { currency, setCurrency, format, options: currencyOptions } = useCurrency();
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        setNavCategories((data || []).filter((item) => item.isActive ?? item.is_active ?? true));
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const NAV_LINKS = [
    ...
    ...navCategories.map((category) => ({ label: category.name, href: `/categories/${category.id}` })),
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center gap-4">
            ...
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-lg border border-beige-200 z-50 overflow-hidden">
              {searchLoading ? (
                ...
                <>
                  {searchResults.map(p => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{(p as any).brands?.name} · {format(Number(p.price) || 0)}</p>
                      </div>
                    </Link>
                  ))}
                  <button
                  ...
                  >
                    {t('nav_search_results_link')} &lsquo;{searchQuery}&rsquo;
                  </button>
                </>
              ) : (
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <div ref={currencyMenuRef} className="relative">
            ...
            >
              <Coins size={17} />
              <span className="text-xs font-sans font-semibold hidden sm:block">{currency}</span>
            </button>
            {currencyMenuOpen && (
              <div className="absolute right-0 top-full mt-1 bg-white border border-beige-200 rounded-lg shadow-lg z-50 overflow-hidden min-w-[160px] max-h-72 overflow-y-auto">
                {currencyOptions.map(opt => (
                  <button
                    key={opt.code}
                    onClick={() => { setCurrency(opt.code); setCurrencyMenuOpen(false); }}
                    className={`w-full flex items-center justify-between gap-3 px-4 py-2.5 text-sm font-sans transition-colors hover:bg-beige-50 ${currency === opt.code ? 'text-burgundy-700 font-semibold' : 'text-foreground'}`}
                  >
                    <span>{opt.code}</span>
          ...
          <div ref={userMenuRef} className="relative">
            ...
            >
              <User size={20} />
              <span className="text-[10px] font-sans hidden sm:block">{user ? t('nav_my_account') : t('nav_sign_in')}</span>
            </button>
            {userMenuOpen && (
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-beige-100 bg-white">
          <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-1">
            ...
            <div className="border-t border-beige-100 mt-2 pt-2">
              <p className="px-3 py-1 text-xs font-sans text-muted-foreground uppercase tracking-wide">Currency</p>
              <div className="flex flex-wrap gap-2 px-3 py-2">
                {currencyOptions.map(opt => (
                  <button
                    key={opt.code}
                    onClick={() => { setCurrency(opt.code); setMobileOpen(false); }}
                    className={`px-3 py-1.5 rounded-lg text-xs font-sans font-medium transition-colors ${currency === opt.code ? 'bg-burgundy-700 text-white' : 'bg-beige-50 text-foreground hover:bg-beige-100'}`}
                  >
                    {opt.code}
                  </button>
                ))}
