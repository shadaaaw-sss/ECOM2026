  ...
  const handleCoupon = async () => {
    ...
    try {
      ...
      if (coupon.min_order_amount && subtotal < coupon.min_order_amount) {
        setCouponError(`Minimum order of ${format(Number(coupon.min_order_amount) || 0)} required`);
        return;
      }
                  ...
                  {shippingMethods.length > 0 ? (
                    shippingMethods.map(method => (
                      <label key={method.id} className={`flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-all ${shippingMethod === method.name ? 'border-burgundy-700 bg-burgundy-50' : 'border-beige-200 hover:border-beige-300'}`}>
                        ...
                        <span className="font-sans font-semibold text-sm">{method.price === 0 ? t('checkout_free') : format(Number(method.price) || 0)}</span>
                      </label>
                    ))
                  ) : (
            ...
            <div>
              <div className="bg-white rounded-xl border border-beige-100 p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-5 max-h-56 overflow-y-auto">
                  {items.map(({ product, quantity }) => (
                    <div key={product.id} className="flex gap-3 items-start">
                      ...
                      <span className="text-xs font-semibold font-sans flex-shrink-0">{format(Number(product.price) * quantity)}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mb-5 pb-5 border-b border-beige-100">
                  ...
                  {discount > 0 && <p className="text-xs text-green-600 font-sans mt-1">{t('checkout_coupon_applied')} -{format(discount)}</p>}
                </div>
                ...
                <div className="space-y-2 mb-5 text-sm font-sans">
                  <div className="flex justify-between"><span className="text-muted-foreground">{t('checkout_subtotal')}</span><span>{format(subtotal)}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">{t('checkout_shipping')}</span><span>{shippingFee === 0 ? t('checkout_free') : format(shippingFee)}</span></div>
                  {discount > 0 && <div className="flex justify-between text-green-600"><span>{t('checkout_discount')}</span><span>-{format(discount)}</span></div>}
                </div>
                <div className="border-t border-beige-100 pt-4 mb-5">
                  <div className="flex justify-between items-center">
                    <span className="font-semibold font-sans">{t('checkout_total')}</span>
                    <span className="font-serif text-2xl font-bold">{format(total)}</span>
                  </div>
                </div>
