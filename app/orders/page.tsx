              ...
              <div key={order.id} className="bg-white rounded-xl border border-beige-100 overflow-hidden">
                <div className={`p-5 flex items-start ${isRTL ? 'flex-row-reverse' : ''} justify-between gap-4`}>
                  ...
                  <div className={`flex-shrink-0 ${isRTL ? 'text-left' : 'text-right'}`}>
                    <p className="font-serif font-bold text-lg">{format(Number(order.total) || 0)}</p>
                    <button
                ...
                {expanded === order.id && order.order_items && (
                  <div className="border-t border-beige-100 p-5 bg-beige-50">
                      ...
                      {order.order_items.map(item => (
                        <div key={item.id} className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                          ...
                          <div className={`text-sm font-sans ${isRTL ? 'text-left' : 'text-right'}`}>
                            <p className="font-medium">{format(Number(item.subtotal) || 0)}</p>
                    ...
                    <div className="border-t border-beige-200 pt-3 space-y-1 text-xs font-sans">
                      <div className={`flex justify-between text-muted-foreground ${isRTL ? 'flex-row-reverse' : ''}`}><span>{t('checkout_subtotal')}</span><span>{format(Number(order.subtotal) || 0)}</span></div>
                      <div className={`flex justify-between text-muted-foreground ${isRTL ? 'flex-row-reverse' : ''}`}><span>{t('checkout_shipping')}</span><span>{order.shipping_fee === 0 ? t('checkout_free') : format(Number(order.shipping_fee) || 0)}</span></div>
                      {order.discount_amount > 0 && <div className={`flex justify-between text-green-600 ${isRTL ? 'flex-row-reverse' : ''}`}><span>{t('checkout_discount')}</span><span>-{format(Number(order.discount_amount) || 0)}</span></div>}
                      <div className={`flex justify-between font-bold text-sm text-foreground border-t border-beige-200 pt-2 mt-2 ${isRTL ? 'flex-row-reverse' : ''}`}><span>{t('checkout_total')}</span><span>{format(Number(order.total) || 0)}</span></div>
                    </div>
                  </div>
                )}
              </div>
