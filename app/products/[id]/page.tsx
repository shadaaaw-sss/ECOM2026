...
export default function ProductDetailPage() {
  ...
  const images = (product as any).images?.length
    ...
    : product.thumbnailUrl || product.thumbnail_url
      ? [{ url: product.thumbnailUrl || product.thumbnail_url, type: 'image', isMain: true } as any]
      : [];
    ...
    <div className="min-h-screen bg-cream py-6" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mb-14">
          {/* Gallery */}
          <ProductGallery media={images} productName={product.name} />
          ...
          <div>
            ...
            <div className={`flex items-baseline gap-3 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <span className="text-3xl font-bold font-sans text-foreground">{format(Number(product.price) || 0)}</span>
              {(product as any).original_price && (
                <span className="text-lg font-sans text-muted-foreground line-through">{format(Number((product as any).original_price) || 0)}</span>
              )}
              {discountAmount > 0 && (
            ...
            {savings > 0 && (
              <p className="text-green-600 text-sm font-sans font-medium mb-4">
                {t('product_you_save')} {format(Number(savings) || 0)}
              </p>
            )}
