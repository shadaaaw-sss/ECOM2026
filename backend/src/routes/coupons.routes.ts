...
couponsRoutes.get('/:code', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM coupon WHERE code = $1 AND (starts_at IS NULL OR starts_at <= NOW()) AND (ends_at IS NULL OR ends_at >= NOW())',
...
couponsRoutes.post('/', authMiddleware, requireAdmin, async (req, res) => {
  ...
  try {
    const id = uuidv4();
    const { rows } = await pool.query(
      'INSERT INTO coupon(id, code, discount_percent, starts_at, ends_at, created_at) VALUES($1,$2,$3,$4,$5,NOW()) RETURNING *',
      [id, String(code).toUpperCase(), Number(discountPercent), startsAt || null, endsAt || null]
    );
