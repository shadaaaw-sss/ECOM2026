...
wishlistRoutes.get('/', authMiddleware, async (req, res) => {
    ...
    try {
        const { rows } = await pool.query('SELECT w.id as wishlist_id, w.created_at as wishlist_created_at, p.* FROM wishlist_item w LEFT JOIN product p ON w.product_id = p.id WHERE w.user_id = $1', [userId]);
